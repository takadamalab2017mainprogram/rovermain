...
// Debug logging
void PWM::set_loglevel(int level)
{
	log_level = level;
}

static void PWM::log_debug(char* fmt, ...)
{
...
static void PWM::gpio_set_mode(uint32_t pin, uint32_t mode)
{
...
static void PWM::gpio_set(int pin, int level)
{
	if (level)
...
static void PWM::init_gpio(int gpio)
{
...
static void PWM::terminate(void)
{
	shutdown();
	exit(EXIT_SUCCESS);
}
...
// on process exit!
static void PWM::setup_sighandlers(void)
{
	int i;
	for (i = 0; i < 64; i++) {
		struct sigaction sa;
...
// Memory mapping
static uint32_t PWM::mem_virt_to_phys(int channel, void *virt)
{
...
static void * PWM::map_peripheral(uint32_t base, uint32_t len)
{
...
uint8_t* PWM::get_cb(int channel)
{
...
int PWM::clear_channel(int channel)
{
	int i;
...
int PWM::clear_channel_gpio(int channel, int gpio)
{
	int i;
	uint32_t *dp = (uint32_t *)channels[channel].virtbase;
...
int PWM::add_channel_pulse(int channel, int gpio, int width_start, int width)
{
	int i;
	uint32_t phys_gpclr0 = 0x7e200000 + 0x28;
...
static int PWM::make_pagemap(int channel)
{
	int i, fd, memfd, pid;
	char pagemap_fn[64];
...
static int PWM::init_virtbase(int channel)
{
...
static int PWM::init_ctrl_data(int channel)
{
	dma_cb_t *cbp = (dma_cb_t *)get_cb(channel);
...
static void PWM::init_hardware(void)
{
	if (delay_hw == DELAY_VIA_PWM) {
		// Initialise PWM
		pwm_reg[PWM_CTL] = 0;
...
// added at any time.
int PWM::init_channel(int channel, int subcycle_time_us)
{
...
int PWM::print_channel(int channel)
{
	if (channel > DMA_CHANNELS - 1)
...
void PWM::set_softfatal(int enabled)
{
	soft_fatal = enabled;
}

char * PWM::get_error_message(void)
{
...
int PWM::setup(int pw_incr_us, int hw)
{
	delay_hw = hw;
...
int PWM::is_setup(void)
{
	return _is_setup;
}

int PWM::is_channel_initialized(int channel)
{
	return channels[channel].virtbase ? 1 : 0;
}

int PWM::get_pulse_incr_us(void)
{
	return pulse_width_incr_us;
}

int PWM::get_channel_subcycle_time_us(int channel)
{
