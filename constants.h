...
//111111.1m = 1度、厳密言うと、経度（縦）は一定の比率だが、緯度（横）は赤道の値になる、高緯度だと、一度の距離が短くなる
...
const static double NAVIGATING_DIRECTION_UPDATE_INTERVAL = 5;//進行方向を変更する間隔(秒) 2016/08/31 5->1
const static double NAVIGATING_MAX_DELTA_DIRECTION = 90;//一回の操作で方向転換する最大の角度
const static double NAVIGATING_STUCK_JUDGEMENT_THRESHOLD = 5/ 111111.1; // NAVIGATING_DIRECTION_UPDATE_INTERVALの間に移動した距離がこの閾値以下ならスタック判定とする
