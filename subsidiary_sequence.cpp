...
void Escaping::onUpdate(const struct timespec& time)
{
	...
	case STEP_PRE_CAMERA:
		//âÊëúéBâeópÇ…ãNÇ´è„Ç™ÇËìÆçÏÇçsÇ¢ÅAêîïbë“ã@Ç∑ÇÈ
		if(gWakingState.isActive())mLastUpdateTime = time;//ãNÇ´è„Ç™ÇËìÆçÏíÜÇÕë“ã@Ç∑ÇÈ
		if(Time::dt(time,mLastUpdateTime) > 2)//ãNÇ´è„Ç™ÇËäÆóπå„ÅAàÍíËéûä‘Ç™åoâﬂÇµÇƒÇ¢ÇΩÇÁ
		{
		...
		break;
	case STEP_CAMERA:
		//âÊëúèàóùÇçsÇ¢ÅAç°å„ÇÃçsìÆÇåàíËÇ∑ÇÈ
		if(Time::dt(time,mLastUpdateTime) >= 2)
		{
	...
	case STEP_CAMERA_TURN:
		//âÊëúèàóùÇÃåãâ ÅAâÒì]Ç∑ÇÈïKóvÇ™Ç†Ç¡ÇΩèÍçá
	...
	case STEP_CAMERA_FORWARD:
		//âÊëúèàóùÇÃåãâ ÅAíºêiÇ∑ÇÈïKóvÇ™Ç†Ç¡ÇΩèÍçá
	...
	case STEP_CAMERA_TURN_HERE:
		//âÊëúèàóùÇÃåãâ ÅAÇªÇÃèÍâÒì]Ç∑ÇÈïKóvÇ™Ç†Ç¡ÇΩèÍçá
		if(Time::dt(time,mLastUpdateTime) > 0.4 || abs(gGyroSensor.getRz() - mAngle) > 70)
		{
			gCameraCapture.startWarming();
			mCurStep = STEP_BACKWARD;
...
void EscapingRandom::onUpdate(const struct timespec& time)
{
	...
	//		mLastUpdateTime = time;
	//		gMotorDrive.drive(100,-100);
	//		gStabiServo.start(0);					//ÉXÉ^ÉrÇΩÇΩÇﬁ
	//	}
	//	break;
	case STEP_TURN:
		//ÇªÇÃèÍâÒì]ÇçsÇ§
		if(Time::dt(time,mLastUpdateTime) >= 3)
		{
			...
			mLastUpdateTime = time;
			gMotorDrive.drive(100);
			gStabiServo.start(STABI_BASE_ANGLE);	//ÉXÉ^ÉrêLÇŒÇ∑
		}
		break;
	case STEP_FORWARD:
		//ëOêiÇçsÇ§
...
bool Waking::onInit(const struct timespec& time)
{
	...
	gMotorDrive.drive(-mStartPower);	//8-9 chou „Éû„Ç§„Éä„Çπ„Å´„Åó„Å?//ÉÇÅ[É^èoóÕ
	gGyroSensor.setRunMode(true);
	...
	//backstabi í«â¡ backstabiÇâ∫ÇÎÇ∑

	gBackStabiServo.moveHold();  //

	//softcameraservo í«â¡ Ç∆ÇËÇ†Ç¢Ç∏Å@Ç§Ç±Ç™Ç»Ç¢Ç≈
	gSoftCameraServo.moveHold();

	//ëOstabi Ç∆ÇËÇ†Ç¢Ç∏Å@stop
...
void Waking::onUpdate(const struct timespec& time)
{
	...
	switch(mCurStep)//ãNÇ´è„Ç™ÇËäJénÇ™åüímÇ≥ÇÍÇΩèÍçá
	{
	case STEP_STOP:
		if(Time::dt(time,mLastUpdateTime) > 2)//2ïbÇ‹ÇÌÇµÇƒÇ‡íÖínÇ™åüímÇ≥ÇÍÇ»Ç¢èÍçáÇÕÇ†Ç´ÇÁÇﬂÇÈ
		{
			Debug::print(LOG_SUMMARY, "Waking Timeout : unable to land\r\n");
			setRunMode(false);
			gMotorDrive.drive(0);
		}
		if(gAccelerationSensor.getPhi() < mAngleThreshold)	//äpìxÇ™àÍíËà»â∫Ç…Ç»Ç¡ÇΩÇÁíÖínÇ∆îªíË(â¡ë¨ìxÉZÉìÉTÇçÃóp)
		{
		...
		//âÒì]ÇµÇΩäpìxÇ…âûÇ∂ÇƒÉÇÅ[É^ÇÃèoóÕÇïœâªÇ≥ÇπÇÈ
	...
	double dt;
	case STEP_START:
		if(Time::dt(time,mLastUpdateTime) > 0.5)//àÍíËéûä‘âÒì]Ç™åüímÇ≥ÇÍÇ»Ç¢èÍçáÅ®âÒì]ïsâ¬î\Ç∆îªíf
		{
		...
		if(abs(gGyroSensor.getRvx()) > WAKING_THRESHOLD)//âÒì]Ç™åüímÇ≥ÇÍÇΩèÍçáÅ®ãNÇ´è„Ç™ÇËäJénÇµÇΩÇ∆îªíf(ÉWÉÉÉCÉçÇçÃóp) waking_threshold =200
		{
	...
	case STEP_DEACCELERATE:	//Ç‰Ç¡Ç≠ÇËå∏ë¨Ç∑ÇÈ
...
if(dt > mDeaccelerateDuration)
		...
		{
			int tmp_power = std::max((int)((1 - dt / mDeaccelerateDuration) * (mStartPower / 2/*2Ç≈äÑÇÈ*/)), 0);
	...
	case STEP_VERIFY:	
		//ãNÇ´è„Ç™ÇËÇ™ê¨å˜ÇµÇΩÇ©î€Ç©Çâ¡ë¨ìxÉZÉìÉTÇ≈åüèÿ
		if(Time::dt(time,mLastUpdateTime) <= 2.5)	//ÉçÅ[ÉoÇÃépê®Ç™à¿íËÇ∑ÇÈÇ‹Ç≈àÍíËéûä‘ë“Ç¬
		{
			return;
		}
		...
		if(gAccelerationSensor.getAz() > 0.0)
		{
			...
			//ãNÇ´è„Ç™Ç¡ÇΩÇÁÅAëOstabi Çâ∫ÇÎÇ∑
			//gStabiServo.start(STABI_WAKING_ANGLE);
			gStabiServo.start(STABI_BASE_ANGLE); // ãNÇ´è„Ç™ÇËê¨å˜ÇµÇΩÇÁÉXÉ^ÉrÇÉxÅ[ÉXÇÃäpìxÇ…ñﬂÇ∑
		...
		else
		{
			...
			power = std::min((unsigned int)100, mStartPower + ((mWakeRetryCount + 1) * 5));	//ééçsâÒêîÇ≤Ç∆Ç…ÉÇÅ[É^èoóÕÇè„Ç∞ÇÈ
			power=-power;
			gMotorDrive.drive(power);
...
void MovementLogging::onUpdate(const struct timespec& time)
{
	...
	//ÉGÉìÉRÅ[É_ÇÃÉçÉOÇï€ë∂
	//ÉåÉVÉIî‰Ç™ïœçXÇ≥ÇÍÇΩÇÁlogÇ…îΩâfÇ∑ÇÈ
	...
	//ÉGÉìÉRÅ[É_ÉpÉãÉXÇÃç∑ï™ílÇÃéÊìæ
	...
	//ÉXÉ^ÉbÉNîªíËÇÃÉeÉXÉg
	if(mPrevDeltaPulseL >= STUCK_ENCODER_PULSE_THRESHOLD && mPrevDeltaPulseR >= STUCK_ENCODER_PULSE_THRESHOLD)	//ëOâÒÇÃÉpÉãÉXêîÇ™Ëáílà»è„
	{
		if(deltaPulseL < STUCK_ENCODER_PULSE_THRESHOLD && deltaPulseR < STUCK_ENCODER_PULSE_THRESHOLD)			//ç°âÒÇÃÉpÉãÉXêîÇ™Ëáílà»â∫
		{
			write(mFilenameEncoder,		"Stuck detected!");
			if(mBuzzerFlag)
			{
				gBuzzer.start(200, 50 ,3);		//ÉXÉ^ÉbÉNîªíË(âπÇñ¬ÇÁÇ∑ÇÃÇ›)
			}
		}
	}
...
bool MovementLogging::onCommand(const std::vector<std::string>& args)
{
	...
	if(args.size() == 2)
	{
		...
		{
			mBuzzerFlag = !mBuzzerFlag;	//flagÇÃêÿÇËë÷Ç¶
			if(mBuzzerFlag)
			{
		...
		{
			mPrintFlag = !mPrintFlag;	//flagÇÃêÿÇËë÷Ç¶
			if(mPrintFlag)
			{
...
/*	if(gEscapingByStabiState.isActive() || gEscapingRandomState.isActive())
	...
	//ÉXÉ^ÉbÉNÉ`ÉFÉbÉNÅDëOâÒÇ™Ëáílà»è„Ç≈ÅCç°âÒÇ™Ëáílà»â∫Ç»ÇÁÉXÉ^ÉbÉNîªíËÇ∑ÇÈ
	if(mPrevDeltaPulseL >= pulse_threshold && mPrevDeltaPulseR >= pulse_threshold)	//ëOâÒÇÃÉpÉãÉXêîÇ™Ëáílà»è„
	{
		if(deltaPulseL < pulse_threshold || deltaPulseR < pulse_threshold)			//ç°âÒÇÃÉpÉãÉXêîÇ™Ëáílà»â∫
		{
			//ÉXÉ^ÉbÉNîªíË
	...
	//mCurrentMaxPulseÇÊÇËëÂÇ´ÇØÇÍÇŒmCurrentMaxPulseÇçXêVÇ∑ÇÈ
...
{
	//ÉpÉãÉXêîÇ™Ç†Ç‹ÇËÇ…Ç‡è¨Ç≥Ç¢(orëÂÇ´Ç¢)èÍçáÇÕñ≥éãÇ∑ÇÈ
	...
	//Ç†Ç‹ÇËÇ…Ç‡ëÂÇ´Ç≠ËáílÇ™çXêVÇ≥ÇÍÇÈèÍçáÇÕñ≥éãÇ∑ÇÈ
